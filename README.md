...
Run `npm ci`, then `npm run dev`. Verify changes with `npm test` and `npm run build`; browser regressions are available with `npm run test:browser`.

The viewer uses a patched OCCT WebAssembly runtime. Its source revisions, patch, rebuild instructions, and licenses are in [vendor/occt-js](vendor/occt-js). The site also distributes these materials under `cad-source/`.

See the [kernel evaluation](docs/kernel-evaluation.md) and [performance measurements](docs/performance.md) for the reasons behind the implementation and reproducible benchmarks.
